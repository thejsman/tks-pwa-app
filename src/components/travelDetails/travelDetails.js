...
import {
  fetchGuestFamily,
  uploadArrivalTravelPhoto,
  uploadDepartureTravelPhoto,
  submitGuestDetails,
  guestClearPassport,
...
class TravelDetails extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.guestInformation !== nextProps.guestInformation) {
      ...
      guestInformation &&
        guestInformation.filter(gt => {
          if (
            gt._id ===
            (this.props.guestInformation && this.props.guestInformation._id)
          ) {
            return gt;
          }
        });
  ...
  clearTickets() {
    ...
    if (arrivalTickets.length > 0) this.clearArrivalTickets();
    if (departureTickets.length > 0) this.clearDepartureTickets();
  }
  saveArrivalTickets() {
    ...
    let self = this;
    uploadArrivalTravelPhoto(
      this.state.arrivalTickets,
      self.getArrivalTickets,
      updatedGuestDetails
    );
  }
  clearArrivalTickets() {
    this.setState({ loading: true });
    let updatedGuestDetails = Object.assign({}, this.state.guestInformation);
    removeTravelTickets(updatedGuestDetails._id, "arrival")
      .then(() => {
  ...
  clearDepartureTickets() {
    this.setState({ loading: true });
    let updatedGuestDetails = Object.assign({}, this.state.guestInformation);
    removeTravelTickets(updatedGuestDetails._id, "departure")
      .then(() => {
        this.setState({ loading: false });
        this.setState({
          guestInformation: Object.assign({}, this.state.guestInformation, {
  ...
  getArrivalTickets(tickets, updatedGuestDetails) {
    ...
    this.setState({
      guestInformation: Object.assign({}, this.state.guestInformation, {
        TicketsArrival: tickets,
      }),
      arrivalTickets: tickets,
    });
  ...
  saveDepartureTickets() {
    ...
    let self = this;
    uploadArrivalTravelPhoto(
      this.state.departureTickets,
      self.getDepartureTickets,
      updatedGuestDetails
    );
  }
  ...
  getDepartureTickets(tickets, updatedGuestDetails) {
    ...
    this.setState({
      guestInformation: Object.assign({}, this.state.guestInformation, {
        TicketsDeparture: tickets,
      }),
      departureTickets: tickets,
    });
    //console.log(this.state.guestInformation);
  }
  ...
  getGuestFamilyDetails(event) {
    ...
    return this.setState({
      ...
      arrivalTickets: arrival,
      departureTickets: departure,
    });
  }
  render() {
    $(".backIcon").show();
      ...
      <div className="container">
        ...
        <div className="row">
          <div className="col-sm-12">
            ...
            <div
              className="BtnCommon responsiveBtn rsvpWeddingGuestName"
              style={{ marginTop: "90px" }}
            >
              <button className="btn appBodyFontFamily appBodyFontColor commonBtnDestination active">
                FLIGHTs
              </button>
          ...
          <div className="col-sm-12" style={{ marginTop: "20px" }}>
            <div className="appGradientColor appBodyFontColor appBodyFontFamily">
              ...
              <div className="row">
                <div className="col-md-6 travelP">
                  <div className="travelUpload">
                    <p>ARRIVAL</p>
                    <p>ADD TICKET</p>
                    {this.state.arrivalTickets || arrivalTickets.length > 0 ? (
                      <label
                        className="btn-bs-file myInformationBtn ankur appBodyFontFamily appBodyFontColor active abl-filename"
                        id="photoLabel"
                      >
                        <span>Arrival Tickets</span>
                      </label>
                    ) : null}
                    {this.state.arrivalTickets ||
                    arrivalTickets.length > 0 ? null : (
                      <div>
                        <label
                  ...
                  <div className="travelUpload">
                    ...
                    departureTickets.length > 0 ? (
                      <label
                        ...
                        id="photoLabel"
                      >
                        <span>Departure Tickets</span>
                      </label>
                    ) : null}
                    {this.state.departureTickets ||
                    departureTickets.length > 0 ? null : (
                      <div>
                        <label
                          className="btn-bs-file"
                          style={{ marginTop: "10px" }}
              ...
              <div className=" row submit-btn" style={{ marginTop: "30px" }}>
                <button
                  ...
                  onClick={() => {
                    dept.length > 0 || arrival.length > 0
                      ? this.clearTickets()
                      : this.saveTickets();
                  }}
                >
                  {dept.length > 0 || arrival.length > 0 ? "CLEAR" : "SAVE"}
                </button>
              </div>
            </div>
